...
module Twitter
  ...
  class Regex
    ...
    if major.to_i >= 1 && minor.to_i >= 9
      ...
      REGEXEN[:list_name] = Regexp.compile("^[a-zA-Z\x80-\xff].{0,79}$")
    end
    ...
    if major.to_i >= 1 && minor.to_i >= 9
      ...
      REGEXEN[:auto_link_usernames_or_lists] = Regexp.compile("([^a-zA-Z0-9_]|^)([@＠]+)([a-zA-Z0-9_]{1,20})(\/[a-zA-Z][a-zA-Z0-9\x80-\xff\-]{0,79})?")
    end
